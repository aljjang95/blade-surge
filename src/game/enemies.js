...
export class Enemy extends Actor {
  constructor(game, gltf, weaponsGltf, def, scaleMult, pos) {
    super(game, gltf, { scale: def.scale, tint: def.tint || (def.boss ? '#ffb0b0' : null) });
    ...
    if (def.elite) this.tintEmissive = new THREE.Color(0.12, 0.08, 0.0);
    ...
    this.play('Spawn_Ground', { once: true, fade: 0, speed: 1.7 });
    this.spawnLen = (this.clips['Spawn_Ground']?.duration || 1) / 1.7;
  ...
  hurt(dmg, { crit = false, dirx = 0, dirz = 0, kb = 2, stun = 0, kind = 'slash', up = false, slow = 0, poison = false } = {}) {
    ...
    if (this.def.dodge && Math.random() < this.def.dodge && this.state === 'chase' && !this.stun) {
      this.play('Dodge_Backward', { once: true, fade: 0.05, speed: 1.5 }); this.state = 'dodge'; this.stateT = 0; this.kb.set(-dirx, 0, -dirz).normalize().multiplyScalar(-8);
    ...
    if (staggerOK && !warrior && this.state !== 'dead') {
      if (this.state !== 'attack' || kb >= 4) { this.state = 'hurt'; this.stateT = 0; this.stagger = 0.22 + Math.min(0.4, kb * 0.03); this.play(Math.random() < 0.5 ? 'Hit_A' : 'Hit_B', { once: true, fade: 0.04, speed: 1.8 }); this.telegraph = 0; this.attackDone = false; }
    }
  ...
  update(dt) {
    ...
    if (this.state === 'spawn') {
      if (this.stateT > this.spawnLen * 0.8) { this.spawning = false; this.state = 'chase'; this.play(this.clips['Idle_Combat'] ? 'Idle_Combat' : 'Idle', { fade: 0.2 }); if (this.isBoss) { this.game.renderer.shake(0.6); audio.play('hit_bell', { vol: 0.6, rate: 0.5 }); } }
      return;
    }
    if (this.stun > 0 && this.state !== 'dead') { this.vel.set(0, 0, 0); if (this.actionName !== 'Hit_A' && this.actionName !== 'Hit_B' && this.state !== 'hurt') { this.play('Hit_A', { once: true, clamp: true, fade: 0.05 }); } this.telegraph = 0; this.attackDone = false; if (this.state === 'attack') this.state = 'chase'; return; }
    if (this.isBoss) {
    ...
    if (this.state === 'hurt') { this.vel.set(0, 0, 0); if (this.stateT > this.stagger) { this.state = 'chase'; this.play(this.clips['Idle_Combat'] ? 'Idle_Combat' : 'Idle', { fade: 0.12 }); } return; }
    if (this.state === 'dodge') { if (this.stateT > 0.4) { this.state = 'chase'; this.play('Idle', { fade: 0.1 }); } return; }
    ...
    if (this.state === 'chase') {
      ...
      if (d > range * 0.92 && !(this.def.ranged && d < range * 0.6)) {
        ...
        const run = this.def.spd < 4 ? 'Walking_D_Skeletons' : 'Running_A';
        if (this.actionName !== run) this.play(run, { fade: 0.15, speed: spdMul * (this.def.spd > 5 ? 1.2 : 1) });
      } else if (this.def.ranged && d < range * 0.45) {
        const spd = this.def.spd * 0.8; this.vel.set(-dx / d * spd + sx, 0, -dz / d * spd + sz); if (this.actionName !== 'Walking_Backwards') this.play('Walking_Backwards', { fade: 0.15 });
      } else {
        this.vel.set(sx, 0, sz);
        if (this.atkCd <= 0 && p.alive) this.startAttack(d);
        else if (this.actionName !== 'Idle_Combat' && this.actionName !== 'Idle') this.play(this.clips['Idle_Combat'] ? 'Idle_Combat' : 'Idle', { fade: 0.15 });
      }
      ...
      if (t >= 1) { this.state = 'chase'; this.dashV = null; this.atkCd = (this.def.atkTime * 0.6 + Math.random() * 0.8) * (this.enraged ? 0.6 : 1); this.play(this.clips['Idle_Combat'] ? 'Idle_Combat' : 'Idle', { fade: 0.15 }); }
    }
  }
  startAttack(d) {
    this.state = 'attack'; this.stateT = 0; this.attackDone = false;
    let anim = this.def.anim; this.special = null;
    if (this.isBoss) {
      const r = Math.random(); const kit = this.def.kit;
      if (kit === 'warlord') { if (r < 0.3) { anim = '2H_Melee_Attack_Spin'; this.special = 'spin'; } else if (r < 0.55) { anim = '1H_Melee_Attack_Jump_Chop'; this.special = 'slam'; } else if (this.phase >= 1 && r < 0.7) { anim = 'Spellcast_Summon'; this.special = 'summon'; } }
      else if (kit === 'lich') { if (r < 0.35) { anim = 'Spellcast_Raise'; this.special = 'soulrain'; } else if (this.phase >= 1 && r < 0.55) { anim = 'Spellcast_Summon'; this.special = 'summon'; } else { this.special = 'fan'; } }
      else if (kit === 'reaper') { if (r < 0.45 && d > 3) { anim = 'Dodge_Forward'; this.special = 'dash'; } else if (r < 0.7) { anim = '2H_Melee_Attack_Spin'; this.special = 'spin'; } else if (this.phase >= 1 && r < 0.8) { anim = 'Spellcast_Summon'; this.special = 'summon'; } }
    }
  ...
  doAttack() {
    ...
    if (this.special === 'fan') {
      for (let i = -1; i <= 1; i++) { const dir = p.pos.clone().setY(1.2).sub(this.pos.clone().setY(1.5)).normalize().applyAxisAngle(new THREE.Vector3(0, 1, 0), i * 0.35); const sp = this.pos.clone().addScaledVector(f, 0.8).setY(1.5 * this.def.scale); g.spawnProjectile({ pos: sp, dir, speed: 12, radius: 0.7, dmg: dmg * 0.8, color: 0x60ff80, size: 0.55, owner: this, kb: 3, kind: 'magic', life: 1.6, trail: 0x60ff80, hostile: true }); }
    ...
    if (this.def.ranged) {
      ...
      g.spawnProjectile({ pos: sp, dir, speed: 11, radius: 0.7, dmg, color: 0x60ff80, size: 0.5, owner: this, kb: 3, kind: 'magic', life: 1.6, trail: 0x60ff80, hostile: true });
