...
ART=ROOT/'art/oathhall-v1'; OUT=ROOT/'public/models/oathhall-v1'; EVIDENCE=ROOT/'work/visual-v1'
for p in [ART,OUT,EVIDENCE]: p.mkdir(parents=True,exist_ok=True)
bpy.ops.wm.read_factory_settings(use_empty=True)
scene=bpy.context.scene
source=bpy.data.collections.new('OATH_HALL | editable stonework');scene.collection.children.link(source)
...
bpy.ops.object.select_all(action='DESELECT');scene['description']='Oath Hall V1 / TLL / original authored architecture; character assets unmodified'
bpy.ops.wm.save_as_mainfile(filepath=str(ART/'oathhall-v1.blend'))
bpy.ops.render.render(write_still=True)
triangles=sum(sum(len(p.vertices)-2 for p in o.data.polygons) for o in exports)
receipt={'blender':bpy.app.version_string,'generator':str(Path(__file__).relative_to(ROOT)),'licence':'Original authored for TLL; no external assets','mesh_batches':len(exports),'materials':len(mats),'triangles':triangles,'bytes':file.stat().st_size,'sha256':hashlib.sha256(file.read_bytes()).hexdigest(),'editable':str((ART/'oathhall-v1.blend').relative_to(ROOT)),'source_object_count':len(source.objects),'hero_modified':False}
