...
import { ITEM_POOL, ITEM_BY_ID, SLOTS, SETS, itemStats, enhanceCost, enhanceStones, enhanceChance, destroyChance, ENH_MAX, RARITY_WEIGHT_STAGE, RARITY_WEIGHT_GACHA } from '../data/items.js';
...
export class Economy {
  ...
  fresh() {
    return {
      created: now(), name: '보스', gold: 12000, gems: 1500, energy: ENERGY.max, energyT: now(), tickets: 5, ssrTickets: 0, sweep: 3, stones: 12, protect: 1, bless: 1,
      ...
      quests: { kills: 0, stages: 0, pulls: 0, claimed: [] }, settings: { sfx: true, music: true, haptics: true, quality: 'auto' }, limitedStart: now(),
    };
  }
  ...
  heroEquipBonus(id) {
    const h = this.hero(id); const b = { atk: 0, hp: 0, def: 0, crit: 0, atkPct: 0, hpPct: 0, critDmg: 0, ultGain: 0, sets: {} };
    for (const sl of SLOTS) { const iid = h.equip[sl]; const inst = this.s.inventory.find((x) => x.uid === iid); if (inst) { const st = itemStats(inst); b.atk += st.atk; b.hp += st.hp; b.def += st.def; b.crit += st.crit; const set = ITEM_BY_ID[inst.id].rarity; b.sets[set] = (b.sets[set] || 0) + 1; } }
    b.active = [];
    for (const rar in b.sets) { const n = b.sets[rar]; const S = SETS[rar]; const apply = (o, tier) => { b.atkPct += o.atk || 0; b.hpPct += o.hp || 0; b.crit += o.crit || 0; b.critDmg += o.critDmg || 0; b.ultGain += o.ultGain || 0; b.active.push({ set: S, tier, n }); }; if (n >= 4) apply(S.four, 4); else if (n >= 2) apply(S.two, 2); }
    return b;
  }
  ...
  enhance(uid, { protect = false, bless = false } = {}) {
    ...
    if (this.s.stones < stones) return { ok: false, reason: 'stones' };
    if (protect && this.s.protect <= 0) return { ok: false, reason: 'protect' };
    if (bless && this.s.bless <= 0) return { ok: false, reason: 'bless' };
    this.s.gold -= cost; this.s.stones -= stones; if (protect) this.s.protect--; if (bless) this.s.bless--;
  ...
  completeStage(stage, stars, { double = false, fieldGold = 0, fieldStones = 0, fieldLoot = [] } = {}) {
    ...
    const got = this.addRewards({ gold: r.gold * m + fieldGold * m, gems: first ? r.firstGems : 0, stones: (r.stones || 0) * m + fieldStones }, { silent: true });
...
export const REWARD_LABEL = { gold: ['골드', '/img/icon_gold.webp'], gems: ['보석', '/img/icon_gem.webp'], energy: ['에너지', '/img/icon_energy.webp'], tickets: ['소환권', '/img/icon_ticket.webp'], ssrTicket: ['SSR 확정권', '/img/icon_ticket_ssr.webp'], sweep: ['소탕권', '/img/icon_sweep.webp'], stones: ['강화석', '/img/icon_stone.webp'], protect: ['보호 주문서', '/img/icon_protect.webp'], bless: ['축복 주문서', '/img/icon_bless.webp'], monthly: ['월정액(일)', '/img/icon_gem.webp'], vip: ['VIP(일)', '/img/icon_vip.webp'] };
