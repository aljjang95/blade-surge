...
export class MasterworksView {
  constructor(app,battle) {
    ...
    for(const [id,label] of [['build','원정 준비'],['mastery','영구 숙련'],['quests','길드 의뢰'],['journal','발견과 선택']])this.nav.append(btn(label,()=>{this.tab=id;this.render();}));
  ...
  intro(title,text,art='guild_map') {
    const hero=n('section','mw-banner');hero.style.backgroundImage=`linear-gradient(90deg,#101e20ed,#101e2070),url('/img/expansion/${art}.webp')`;
    hero.append(n('small','mw-eyebrow',`길드 명성 ${fmt(this.state.renown)} · 발견 ${this.state.discoveries.length}`),n('h3','',title),n('p','',text));this.content.append(hero);
  }
  renderBuild() {
    const s=this.state,service=this.battle.masterworks;
    this.intro('어떤 모험가가 될 것인가','전투 방식은 자유롭게 바꾸고, 발견과 의뢰로 영구 숙련을 쌓으세요. 각인은 출격 중 선택합니다.');
    this.content.append(n('h3','mw-section-title','전투 방식'));
    const paths=n('div','mw-grid');for(const p of PATHS){const b=btn('',()=>this.act(()=>service.path(p.id)),'mw-card');b.setAttribute('aria-pressed',String(s.path===p.id));b.append(n('small','mw-eyebrow',s.path===p.id?'선택 중':'자유 전환'),n('h4','',p.name),n('p','',p.description));paths.append(b);}this.content.append(paths);
    ...
    const challenges=n('div','mw-grid');for(const c of CHALLENGES){const b=btn('',()=>this.act(()=>service.challenge(c.id)),'mw-card');b.setAttribute('aria-pressed',String(s.challengeIds.includes(c.id)));b.append(n('small','mw-eyebrow',s.challengeIds.includes(c.id)?'서약 활성':'선택 가능한 도전'),n('h4','',c.name),n('p','',c.description));challenges.append(b);}this.content.append(challenges);
    const presets=n('div','mw-presets');s.presets.forEach((p,i)=>{const box=n('div');box.append(n('strong','',p.name),n('small','',`${PATHS.find(x=>x.id===p.path)?.name} · 서약 ${p.challengeIds.length}`),btn('불러오기',()=>this.act(()=>service.usePreset(i))),btn('현재 설정 저장',()=>this.act(()=>service.savePreset(i))));presets.append(box);});this.content.append(n('h3','mw-section-title','원정 준비 저장'),presets);
    this.content.append(n('p','mw-muted','AI 결투장은 기존 공통 규칙을 사용합니다. 각인과 서약은 캠페인·재료 던전에 적용됩니다.'));
    this.content.append(btn('재료 던전으로',()=>this.openExpedition('dungeons'),'mw-primary'));
  }
  renderMastery() {
    this.intro('패배해도, 배운 것은 남는다','처치 5회마다 명성 1, 처음 정화한 구역마다 명성 3. 완료 보상과 길드 의뢰로 다음 성장을 준비하세요.','star_archive');
    ...
    for(const [path,title] of [['assault','공세'],['survival','생존'],['insight','통찰']]) {
      ...
      for(const node of MASTERY_NODES.filter(x=>x.path===path)) {
        ...
        const card=n('article','mw-card');card.append(n('small','mw-eyebrow',`${node.tier}단계`),n('h4','',node.name),n('p','',node.description));
  ...
  renderQuests() {
    this.intro('다음 한 걸음을 정하다','부담 없이 끝낼 수 있는 목표. 조건을 달성한 보상은 직접 수령하고 영구 숙련에 사용하세요.','glass_garden');
    const list=n('div','mw-grid');for(const q of BOUNTIES){const cur=this.state.bounties.counts[q.stat],claimed=this.state.bounties.claimed.includes(q.id);const card=n('article','mw-card');const p=n('progress');p.max=q.target;p.value=cur;p.setAttribute('aria-label',q.name);
      card.append(n('small','mw-eyebrow',`명성 +${q.reward}`),n('h4','',q.name),n('p','',q.description),p,n('p','',`${Math.min(cur,q.target)} / ${q.target}`));
    ...
    this.content.append(n('h3','mw-section-title','원하는 장비에 가까워지는 길'),n('p','mw-muted',`${routes}. 재료를 모아 공방에서 원하는 부위를 직접 제작하세요.`),btn('공방에서 목표 장비 확인',()=>this.openExpedition('forge'),'mw-primary'));
  }
  renderJournal() {
    this.intro('세계는 당신의 선택을 기억한다','첫 만남의 선택은 저장됩니다. 같은 길을 다시 찾으면 그 결과와 작은 쉼터의 도움을 만납니다.','star_archive');
    for(const event of STORY_EVENTS){const c=event.choices.find(c=>c.id===this.state.story[event.id]);const card=n('article','mw-story-record');card.append(n('small','mw-eyebrow',c?'남겨진 선택':'아직 만나지 않은 이야기'),n('h4','',event.name),n('p','',c?c.consequence:'구역을 정화하며 길 위의 사람들을 만나세요.'));if(c)card.append(n('small','mw-muted',`당신의 선택: ${c.name} · 다시 만나면 체력 ${c.effects.heal?12:6}% 회복`));this.content.append(card);}
  ...
  renderRun() {
    ...
    if(offer?.kind==='boon') {
      this.content.append(n('p','mw-eyebrow',`각인 선택 ${run.picked.length+1} · 세 가지 가능성`),n('h3','mw-choice-title','이번 싸움을 바꿀 힘'),n('p','mw-muted','하나를 선택하세요. 서로 다른 계열을 모으면 조합 효과가 열립니다. 같은 각인은 3단계까지 강화됩니다.'));
      const cards=n('div','mw-grid mw-choices');for(const id of offer.ids){const b=BOONS.find(x=>x.id===id),rank=run.picked.filter(x=>x===id).length;const card=btn('',()=>this.act(()=>this.battle.selectBoon(id),'이번 원정에 적용했습니다.'),'mw-card mw-boon');card.dataset.boon=id;card.dataset.family=b.family;const art=n('div','mw-boon-art');art.style.backgroundImage=`url('/img/expansion/${family[b.family][1]}.webp')`;card.append(art,n('small','mw-eyebrow',`${family[b.family][0]} · ${rank?`강화 ${rank} → ${rank+1}`:'새 각인'}`),n('h4','',b.name),n('p','',b.description),n('strong','mw-pick','이 각인 선택'));cards.append(card);}this.content.append(cards);
    ...
    const picked=n('div','mw-picked');for(const b of BOONS){const rank=run.picked.filter(id=>id===b.id).length;if(rank)picked.append(n('span',`mw-chip ${b.family}`,`${b.name} ${rank}`));}this.content.append(picked);
    ...
    this.content.append(n('p','mw-muted','각인은 이번 출격에만 유지됩니다. 명성·영구 숙련·도감·선택 기록은 귀환하거나 패배해도 남습니다. 닫아도 대기 중인 선택은 상단 각인 버튼에서 이어갈 수 있습니다.'));
  }
  refresh() {
