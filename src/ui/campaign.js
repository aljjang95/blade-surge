import { CAMPAIGN, EPILOGUE_ENDING, journalEntries } from '../data/campaign-story.js';
...
export function dungeonBriefHtml(stage) {
  const dungeon = stage.dungeon;
  return dungeon ? `<aside class="campaign-dungeon"><b>${storyText(dungeon.name)}</b><p>${storyText(dungeon.route)}</p><small>${dungeon.layout.cells.length}개 구역 · 전투방 정화 후 최종 봉인 해제</small></aside>` : '';
}
...
export function journalHtml(progress) {
  ...
  return `<div class="campaign-journal"><div class="journal-toolbar"><span class="campaign-eyebrow">기억의 기록 · ${entries.length}편</span><button type="button" class="btn btn-ghost btn-sm" id="journal-close">닫기</button></div><h2>${storyText(CAMPAIGN.title)}</h2><p>${storyText(CAMPAIGN.logline)}</p>
    ...
    ${progress.stars?.['6-10'] ? `<article class="campaign-ending"><h3>${storyText(EPILOGUE_ENDING.title)}</h3><p>${storyText(EPILOGUE_ENDING.body)}</p></article>` : ''}
    </div>`;
}

export function resultStoryHtml(stage) {
  return `<span class="campaign-eyebrow">${storyText(stage.code)} · ${stage.finale ? '다섯 서약 완성' : '되찾은 기억'}</span><h3>${storyText(stage.title)}</h3><p>${storyText(stage.story?.aftermath)}</p>${stage.st === 10 && stage.chapter.oath ? `<blockquote>${storyText(stage.chapter.oath)}</blockquote>` : ''}${stage.finale ? `<div class="campaign-ending">${endingHtml()}</div>` : ''}${stage.epilogueFinale ? `<div class="campaign-ending"><h3>${storyText(EPILOGUE_ENDING.title)}</h3><p>${storyText(EPILOGUE_ENDING.body)}</p></div>` : ''}`;
}
