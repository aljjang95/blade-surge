...
test('return build schedules one reverse piercing wave after the baseline slash',()=>{
  ...
  expect(g.shots[0]).toMatchObject({radius:1.55,dmg:48,pierce:true}); expect(g.shots[0].dir.z).toBeLessThan(0);
});

test('fissure build creates three bounded delayed zones without retriggering set procs',()=>{
...
test('pierce build emits one immediate narrow high-speed piercing wave',()=>{
  ...
  expect(g.timers).toHaveLength(0); expect(g.shots).toHaveLength(1); expect(g.shots[0]).toMatchObject({speed:30,radius:.85,dmg:72,pierce:true});
});
