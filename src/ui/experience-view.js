...
export class ExperienceView {
 constructor(app){
  ...
  this.status=node('div','experience-link');this.status.hidden=true;this.status.setAttribute('role','status');this.status.setAttribute('aria-live','polite');document.querySelector('#hud').append(this.status);
