...
export class Battle {
  ...
  // ---------------- 투사체 ----------------
  spawnProjectile({ pos, dir, speed, radius, dmg, color, size = 0.4, owner, kb = 2, kind = 'magic', life = 1.2, pierce = false, trail = null, explode = null, hostile = false, visual = undefined, slow = 0, finisher = false, comboToken = null }) {
    let mesh = null;
    ...
    this.projectiles.push({ pos: pos.clone(), dir: dir.clone().normalize(), speed, radius, dmg, color, owner, kb, kind, life, t: 0, pierce, hit: new Set(), mesh, trail, explode, hostile, slow, finisher, comboToken });
  }
  updateProjectiles(dt) {
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      const p = this.projectiles[i]; p.t += dt; p.pos.addScaledVector(p.dir, p.speed * dt); if (p.mesh) { p.mesh.position.copy(p.pos); if (!p.mesh.userData.arrowVisual) p.mesh.rotation.y += dt * 8; }
      ...
      if (p.hostile) {
        ...
        for (const e of this.enemies) {
          ...
          if (Math.hypot(e.pos.x - p.pos.x, e.pos.z - p.pos.z) < p.radius + e.radius) {
            ...
            this.damageEnemy(e, p.dmg, { kb: p.kb, kind: p.kind, dirx: p.dir.x, dirz: p.dir.z, source: p.owner, slow: p.slow, finisher: p.finisher, comboToken:p.comboToken });
            if (!p.pierce) { done = true; this.fx.burst(p.pos, p.color, { n: 10, speed: 5, size: 0.3 }); break; }
          }
        }
      }
      if (done) { if (p.explode && p.t >= p.life && !p.hit.size) this.explode(p); releaseProjectileVisual(p.mesh); this.projectiles.splice(i, 1); }
    }
  }
  explode(p) {
