...
Integrated onto ac49e5dd (current KayKit ranger, bow, campaign, armor and Flow audio). battle-base.js and save-base.js preserve current main. No character assets changed.

TypeScript, Bun 272 tests, RPG 40 tests and Vite build passed. Tests now accommodate CRLF and distinguish BaseBattle unit fixtures from public RPG damage behavior. Finisher Flow audio routing preserved.

IAB: boot, lobby, 63-species codex, stats, expedition, AUTO combat through 4/11 rooms, Lv.6 EXP63/228, manual pause. Desktop smoke only; all-hero/mobile/long-duration balance matrix remains unverified. Original deployment restriction below is superseded by current owner merge/deploy authorization.
