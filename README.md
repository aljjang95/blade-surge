...
전용 모델은 역할별 필수 클립·무기 소켓·단위 배율을 검증하고 원본 PBR과 재질 배열을 보존합니다. 네 영웅은 기존 KayKit CC0 얼굴·리그·무기를 유지하면서 TLL의 의상·머리 장식·팔다리 외형을 적용합니다. 모델 계약 테스트만으로 실기기 성능이나 완전 독점 원작 캐릭터를 주장하지 않습니다.

## 가로 로비 · 서약의 전당

현재 캐릭터 스타일은 casual-v2입니다. 둥근 왕관·꽃 장식·패딩과 무광 파스텔 색상을 사용하며 로비도 낮은 둥근 아치로 구성합니다. 이전 oath-v1 자산은 보존합니다.

영웅을 가리던 전경 소품과 자동 회전 카메라를 정리하고 고정 3/4 구도의 전당에서 출격합니다. 의상과 관·후드는 같은 전투 리그를 사용하며, 초상은 실제 합성 모델의 렌더입니다. 일반 설정의 개성 동작과 잔불, 장비 등급 효과를 유지하고 동작 줄이기에서는 대기 포즈를 고정합니다. 외형 출처와 재생성 절차는 `public/models/tll/README.md`에 있습니다.

## 게임 루프
```
몹몰이 (10~28마리) → 진공 광역기로 쓸어담기 → 코인·강화석·장비 우수수 드랍
   → 자석 흡수 → 빈 부위는 집는 순간 자동 장착(세트 2조각에서 발동 토스트) → 같은 등급 4개 모아 세트 완성 → 강화(+20, 파괴 위험)
...
| 필드 자동 장착 | 집은 장비가 빈 부위면 그 자리에서 장착 — 스탯·무기/방패 외형·세트 발동이 전투 중 즉시 반영. 설정에서 '빈 슬롯만(기본) / 더 강하면 교체 / 끄기' |
