...
export function applyKnightSlashVariant(game, player, ctx, variantId) {
  ...
  game.fx.light?.(start, 0xffd060, 8, 10, .35); game.renderer?.shake?.(.18);
...
export function installKnightBuildVariants() {
  ...
  SKILLS.holy_slash.cast = function(game, player, ctx) {
    const variant = game?.stage?.party ? 'classic' : knightSlashVariantFromBonus(game?.app?.eco?.heroEquipBonus?.(player.def?.id));
    if (variant === 'pierce') return applyKnightSlashVariant(game, player, ctx, variant);
    baseCast(game, player, ctx);
    applyKnightSlashVariant(game, player, ctx, variant);
  };
  installed = true;
}
