...
export class AppModeView {
  constructor(app) {
    ...
    button.textContent = '앱 화면'; button.setAttribute('aria-label', '주소창 없이 플레이');
    ...
    dialog.querySelector('#app-mode-fullscreen').onclick = () => {
      // Call directly within the tap so the browser's user-activation requirement is met.
      let request;
      try { request = document.fullscreenElement ? document.exitFullscreen()
        : document.documentElement.requestFullscreen({ navigationUI: 'hide' }); }
      catch { this.message('전체 화면을 열지 못했습니다. 홈 화면에 설치해서 실행해 주세요.'); return; }
      Promise.resolve(request).then(() => { this.render(); dialog.close(); }).catch(() => {
        this.message('이 브라우저에서는 전체 화면을 열지 못했습니다. 홈 화면 설치 안내를 이용해 주세요.');
      });
    };
    for (const event of ['fullscreenchange', 'fullscreenerror']) document.addEventListener(event, () => { if (dialog.open) this.render(); });
    window.addEventListener('bladesurge:pwa-state', () => { if (dialog.open) this.render(); });
  }
  ...
  render() {
    ...
    const steps = state.platform === 'ios'
      ? ['Safari에서 이 게임을 여세요.', '공유 버튼 → 홈 화면에 추가를 선택하세요.', '추가를 누른 뒤 홈 화면의 BladeSurge 아이콘으로 실행하세요.']
