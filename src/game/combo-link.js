...
export const COMBO_LINK_RANGE=12;
export class ComboLink {
 ...
 snapshot(now,player){
  const r=this.ready,valid=!!r&&r.player===player&&player?.alive&&r.epoch===(player.knightLifeEpoch||0)&&Number.isFinite(now)&&now<r.until&&!!player.pos&&r.anchor.distanceToSquared(player.pos)<=COMBO_LINK_RANGE**2;
 ...
 consume({player,context,now}){
  ...
  if(!player.pos||r.anchor.distanceToSquared(player.pos)>COMBO_LINK_RANGE**2)return null;
